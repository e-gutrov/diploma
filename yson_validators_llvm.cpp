...
CreateTableSchemaValidator(const TypeBasePtr& schema) {
    ...
    auto module = llvm::parseIRFile(
        "/home/egor/CLionProjects/coursework/"
        "llvm-ir-helpers/llvm_yson_helpers.ll",
        err,
        *context);
    auto functions = GetFunctionAddresses(&builder, module.get());

    auto validateRoot = CreateTypeValidatorNew(
        schema, context.get(), &builder, module.get(), functions);
    ...
    auto initArr = builder.CreateCall(
