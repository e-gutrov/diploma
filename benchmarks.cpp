...
void benchJsonconsValidation(const std::string &schemaName,
...
void benchJsonconsCursorValidation(const std::string &schemaName,
                                   const std::string &data,
...
void benchRapidJsonValidation(const std::string &schemaName,
                              const std::string &data,
...
void benchJsonLlvmValidation(const std::string &schemaName,
                             const std::string &data, const TypeBasePtr &type) {
  auto jit = PrepareJit(PrepareJitFor::Json, false);
  if (auto err =
          jit->addIRModule(JsonValidators::CreateTableSchemaValidator(type))) {
...
void benchYsonValidation(const std::string &schemaName,
                         const std::vector<std::string> &data,
                         const TypeBasePtr &type, const std::string &format) {
  ...
  ankerl::nanobench::Bench().run("YSON polymorphic " + schemaName, [&]() {
    for (const auto &row : data) {
      ...
      NYT::NYson::TYsonPullParser parser(&memoryInput,
...
void benchYsonLlvmValidation(const std::string &schemaName,
                             const std::vector<std::string> &data,
                             const TypeBasePtr &type,
                             const std::string &format) {
  auto jit = PrepareJit(PrepareJitFor::Yson, false);
  if (auto err =
          jit->addIRModule(YsonValidators::CreateTableSchemaValidator(type))) {
    std::cout << toString(std::move(err)) << std::endl;
  }
  auto sym = jit->lookup("main");
  if (!sym) {
...
std::string createListOfAlternatingTokens(int elems,
                                          const std::string &tokenEven,
                                          const std::string &tokenOdd) {
  std::string result = "[";
  for (int i = 0; i < elems; ++i) {
    if (i > 0) {
      result += ", ";
...
void benchListOfListOfOptionalListOfInts(int elems) {
  ...
  std::vector<std::pair<std::string, std::vector<std::string>>> data{
      {"Basic",
       {createListOfAlternatingTokens(elems, "[[1, 2, 3], [], null]", "[[]]")}},
      {"Random", GenerateObjectWithSchema(schema, &g, BATCH_SIZE)},
  };
...
void benchListOfTuplesOfStringIntAndOptionalListOfOptionalStrings(int elems) {
  ...
  auto schema = CreateList(CreateTuple(
      {CreateSimple(ValueType::String), CreateSimple(ValueType::Int),
       CreateOptional(
...
int main() {
  ...
  ankerl::nanobench::Bench().run(
      "SomeFunc", [&]() { ankerl::nanobench::doNotOptimizeAway(SomeFunc(5)); });
