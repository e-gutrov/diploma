...
TEST_CASE("Test int") {
  ...
  SECTION("Test JSON LLVM validation") { testJsonLlvmValidation(tests, type); }

  SECTION("Test YSON cursor validator") {
    testYsonCursorValidation(tests, type);
  }

  SECTION("Test YSON LLVM validation") { testYsonLlvmValidation(tests, type); }
}
...
TEST_CASE("Test list") {
  ...
  auto tests = std::vector<std::pair<std::string, bool>>{
      {"0", false},      {"5", false},
      {"5.1", false},    {R"("abracadabra")", false},
      {"[]", true},      {"[1]", true},
      {"[null]", false}, {"[null, 1, 2, null, 4]", false},
      {"[5.1]", false},  {R"(["1"])", false},
      {"{}", false},     {R"({"x": 1})", false},
  };
...
TEST_CASE("Test list of optional ints") {
  ...
  auto tests = std::vector<std::pair<std::string, bool>>{
      ...
      {"5.1", false},   {R"("abracadabra")", false},
      {"[]", true},     {"[1]", true},
      {"[null]", true}, {"[null, 1, 2, null, 4]", true},
      {"[5.1]", false}, {R"(["1"])", false},
      {"{}", false},
  };
