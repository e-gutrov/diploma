...
namespace {
jsoncons::json
DoGenerateObjectWithSchema(const TypeBasePtr& schema, GeneratorBase* generator);

jsoncons::json
DoGenerateOptional(const TypeBasePtr& schema, GeneratorBase* generator) {
...
jsoncons::json
DoGenerateList(const TypeBasePtr& schema, GeneratorBase* generator) {
    auto length = generator->GenerateListLength();
    std::vector<jsoncons::json> result;
    for (int i = 0; i < length; ++i) {
        result.emplace_back(
            DoGenerateObjectWithSchema(schema->Child(), generator));
    }
    return jsoncons::json(result);
}

jsoncons::json
DoGenerateTuple(const TypeBasePtr& schema, GeneratorBase* generator) {
    std::vector<jsoncons::json> result;
    for (const auto& child : schema->Children()) {
        result.emplace_back(
            DoGenerateObjectWithSchema(child->Schema, generator));
    }
    return jsoncons::json(result);
}

jsoncons::json DoGenerateObjectWithSchema(
    const TypeBasePtr& schema, GeneratorBase* generator) {
...
bool GeneratorBase::GenerateIsNull() {
    return Generator_() % 2;
}
...
bool ConstForOptionsGenerator::GenerateIsNull() {
    return Const_;
}

std::vector<std::string> GenerateObjectWithSchema(
    const TypeBasePtr& schema, GeneratorBase* generator, int minSize) {
