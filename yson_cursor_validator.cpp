...
namespace YsonValidators {
    namespace {
        ...
        public:
            explicit OptionalYsonValidator(std::unique_ptr<PolymorphicValidator> child) : ChildValidator_(
                    std::move(child)) {}
        ...
        public:
            explicit ListYsonValidator(std::unique_ptr<PolymorphicValidator> child) : ChildValidator_(
                    std::move(child)) {}
        ...
        public:
            explicit TupleYsonValidator(std::vector<std::unique_ptr<PolymorphicValidator>> children) : ChildValidators_(
                    std::move(children)) {}
        ...
        public:
            explicit RootYsonValidator(std::unique_ptr<PolymorphicValidator> child) : ChildValidator_(
                    std::move(child)) {}
    ...
    std::unique_ptr<PolymorphicValidator> CreatePolymorphicValidator(const TypeBasePtr &schema, int depth) {
        if (depth == 0) {
            return std::make_unique<RootYsonValidator>(CreatePolymorphicValidator(schema, depth + 1));
        }
        ...
        switch (type) {
            ...
            case ValueType::Optional: {
                return std::make_unique<OptionalYsonValidator>(CreatePolymorphicValidator(schema->Child(), depth + 1));
            }
            case ValueType::List: {
                return std::make_unique<ListYsonValidator>(CreatePolymorphicValidator(schema->Child(), depth + 1));
            }
            case ValueType::Tuple: {
                std::vector<std::unique_ptr<PolymorphicValidator>> children;
                for (const auto &child: schema->Children()) {
                    children.emplace_back(CreatePolymorphicValidator(child->Schema, depth + 1));
                }
