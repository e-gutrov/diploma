...
public:
  ...
  ValueType GetType() const override { return Type_; }
...
public:
  SingleChildType(ValueType t, TypeBasePtr child)
      : Type_(t), Child_(std::move(child)) {}

  ValueType GetType() const override { return Type_; }

  TypeBasePtr Child() override { return Child_; }
...
TypeBasePtr CreateOptional(TypeBasePtr child) {
  return std::make_shared<SingleChildType>(ValueType::Optional,
                                           std::move(child));
}
