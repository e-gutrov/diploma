...
void DoConvertJsonToYson(
    NYson::TYsonWriter* writer, jsoncons::json_cursor* cursor);

void DoConvertJsonListToYson(
    NYson::TYsonWriter* writer, jsoncons::json_cursor* cursor) {
...
std::string
ConvertJsonToYson(const std::string& json, NYT::NYson::EYsonFormat format) {
...
llvm::Function* CreateTypeValidatorNew(
    ...
    switch (schema->GetType()) {
        case ValueType::Optional: {
            auto validateChild = CreateTypeValidatorNew(
                schema->Child(),
                context,
                builder,
                module,
                functions,
                suffix + "OptionalChild");
            ...
            auto nonNullCase =
                llvm::BasicBlock::Create(*context, "nonNull", func);
        ...
        case ValueType::List: {
            ...
            auto entry = llvm::BasicBlock::Create(*context, "entry", func);
            builder->SetInsertPoint(entry);
            auto isBeginArray = builder->CreateCall(
                functions.at("IsBeginArray"), {arrArg, nextArg});

            auto retFalse =
                llvm::BasicBlock::Create(*context, "retFalse", func);
            ...
            auto whileBody =
                llvm::BasicBlock::Create(*context, "whileBody", func);
            builder->CreateBr(while_);
            builder->SetInsertPoint(while_);
            auto isEndArray = builder->CreateCall(
                functions.at("IsEndArray"), {arrArg, nextArg});
        ...
        case ValueType::Object: {
            // TODO: in case of structs, map each field name to a number and
            // lookup it in a map with an external function? based on the index,
            // call the needed function (ptrs will be stored in an array?) and
            // mark the field used Check that all required fields are met
            assert(0);
        }
        case ValueType::Int:
...
llvm::Function* CreateTypeValidator(
    ...
    const std::string& suffix) {
    auto funcType = llvm::FunctionType::get(
        builder->getInt1Ty(), {builder->getInt8PtrTy()}, false);
    auto funcName = "validate" + suffix;
    ...
    switch (schema->GetType()) {
        ...
        case ValueType::Optional: {
            auto validateChild = CreateTypeValidator(
                schema->Child(),
                context,
                builder,
                module,
            ...
            auto isNullCall = builder->CreateCall(
                functions.at("IsNull"), {func->arg_begin()});
            auto nullCase = llvm::BasicBlock::Create(*context, "null", func);
            auto nonNullCase =
                llvm::BasicBlock::Create(*context, "nonNull", func);
            builder->CreateCondBr(isNullCall, nullCase, nonNullCase);
            ...
            builder->SetInsertPoint(nullCase);
            auto nextCall = builder->CreateCall(
                functions.at("CallNext"), {func->arg_begin()});
            builder->CreateRet(builder->getInt1(true));

            builder->SetInsertPoint(nonNullCase);
            auto childCall =
                builder->CreateCall(validateChild, {func->arg_begin()});
            builder->CreateRet(childCall);
            return func;
        }
        case ValueType::List: {
            ...
            builder->SetInsertPoint(entry);
            auto isBeginArray = builder->CreateCall(
                functions.at("IsBeginArray"), {func->arg_begin()});
            ...
            builder->SetInsertPoint(while_);
            auto isEndArray = builder->CreateCall(
                functions.at("IsEndArray"), {func->arg_begin()});
            builder->CreateCondBr(isEndArray, callNextAndRetTrue, whileBody);
        ...
        case ValueType::Tuple: {
            ...
            for (int i = 0; i < childValidators.size(); ++i) {
                ...
                nextField = llvm::BasicBlock::Create(
                    *context, "field" + std::to_string(i + 1), func);
                auto childCall = builder->CreateCall(
                    childValidators[i], {func->arg_begin()});
                builder->CreateCondBr(childCall, nextField, retFalse);
            }
            builder->SetInsertPoint(nextField);
            auto callNextAndRetTrue =
...
orc::ThreadSafeModule FinalizeModule(
    std::unique_ptr<Module> module, std::unique_ptr<LLVMContext> context) {
...
PrepareJit(PrepareJitFor prepareFor, bool useProcessSymbols) {
    ...
    switch (prepareFor) {
        case PrepareJitFor::Json: {
            symbolMap.insert(
                {Mangle("FillWithEvents"),
                 JITEvaluatedSymbol(
                     pointerToJITTargetAddress(&JsonValidators::FillWithEvents),
        ...
        case PrepareJitFor::Yson: {
            symbolMap.insert(
                {Mangle("FillWithEvents"),
                 JITEvaluatedSymbol(
                     pointerToJITTargetAddress(&YsonValidators::FillWithEvents),
                     JITSymbolFlags::Callable)});
            break;
        }
    }
