...
void DoConvertJsonToYson(NYson::TYsonWriter *writer,
                         jsoncons::json_cursor *cursor);

void DoConvertJsonListToYson(NYson::TYsonWriter *writer,
                             jsoncons::json_cursor *cursor) {
  while (cursor->current().event_type() !=
         jsoncons::staj_event_type::end_array) {
    writer->OnListItem();
    DoConvertJsonToYson(writer, cursor);
  }
}

void DoConvertJsonMapToYson(NYson::TYsonWriter *writer,
                            jsoncons::json_cursor *cursor) {
  while (cursor->current().event_type() !=
         jsoncons::staj_event_type::end_object) {
...
GetFunctionAddresses(IRBuilder<> *builder, Module *module) {
  return {
...
llvm::Function *CreateTypeValidatorNew(
    const TypeBasePtr &schema, llvm::LLVMContext *context,
  ...
  auto funcType = llvm::FunctionType::get(builder->getInt1Ty(),
                                          {
                                              int8PtrTp,             // arr
                                              int32Ptr,              // next
                                              int32Ptr,              // size
  ...
  auto newAttrSet =
      attrSet.addFnAttribute(*context, llvm::Attribute::AlwaysInline);
  ...
  case ValueType::Optional: {
    auto validateChild =
        CreateTypeValidatorNew(schema->Child(), context, builder, module,
                               functions, suffix + "OptionalChild");

    auto entry = llvm::BasicBlock::Create(*context, "entry", func);
    builder->SetInsertPoint(entry);
    auto isNullCall =
        builder->CreateCall(functions.at("IsNull"), {arrArg, nextArg});
    ...
    auto nextCall =
        builder->CreateCall(functions.at("CallNext"),
                            {arrArg, nextArg, sizeArg, capacityArg, cursorArg});
    builder->CreateRet(builder->getInt1(true));

    builder->SetInsertPoint(nonNullCase);
    auto childCall = builder->CreateCall(
        validateChild, {arrArg, nextArg, sizeArg, capacityArg, cursorArg});
  ...
  case ValueType::List: {
    auto validateChild =
        CreateTypeValidatorNew(schema->Child(), context, builder, module,
                               functions, suffix + "ListChild");

    auto entry = llvm::BasicBlock::Create(*context, "entry", func);
    builder->SetInsertPoint(entry);
    auto isBeginArray =
        builder->CreateCall(functions.at("IsBeginArray"), {arrArg, nextArg});
    ...
    auto callNextAndRetTrue =
        llvm::BasicBlock::Create(*context, "callNextAndRetTrue", func);
    {
      builder->SetInsertPoint(callNextAndRetTrue);
      auto nextCall = builder->CreateCall(
          functions.at("CallNext"),
          {arrArg, nextArg, sizeArg, capacityArg, cursorArg});
      builder->CreateRet(builder->getInt1(true));
    }
    {
    ...
    auto isEndArray =
        builder->CreateCall(functions.at("IsEndArray"), {arrArg, nextArg});
  ...
  case ValueType::Tuple: {
    ...
    for (const auto &child : schema->Children()) {
      ...
      childValidators.emplace_back(CreateTypeValidatorNew(
          child->Schema, context, builder, module, functions,
    ...
    for (int i = 0; i < childValidators.size(); ++i) {
      ...
      nextField = llvm::BasicBlock::Create(
          *context, "field" + std::to_string(i + 1), func);
      auto childCall =
          builder->CreateCall(childValidators[i], {arrArg, nextArg, sizeArg,
                                                   capacityArg, cursorArg});
  ...
  case ValueType::Object: {
    // TODO: in case of structs, map each field name to a number and lookup it
    // in a map with an external function? based on the index, call the needed
    // function (ptrs will be stored in an array?) and mark the field used Check
    // that all required fields are met
    assert(0);
  }
  case ValueType::Int:
...
llvm::Function *CreateTypeValidator(
  ...
  case ValueType::Optional: {
    auto validateChild =
        CreateTypeValidator(schema->Child(), context, builder, module,
                            functions, suffix + "OptionalChild");
    ...
    auto isNullCall =
        builder->CreateCall(functions.at("IsNull"), {func->arg_begin()});
    auto nullCase = llvm::BasicBlock::Create(*context, "null", func);
    auto nonNullCase = llvm::BasicBlock::Create(*context, "nonNull", func);
    builder->CreateCondBr(isNullCall, nullCase, nonNullCase);
  ...
  case ValueType::List: {
    ...
    builder->SetInsertPoint(entry);
    auto isBeginArray =
        builder->CreateCall(functions.at("IsBeginArray"), {func->arg_begin()});
    ...
    builder->SetInsertPoint(while_);
    auto isEndArray =
        builder->CreateCall(functions.at("IsEndArray"), {func->arg_begin()});
    builder->CreateCondBr(isEndArray, callNextAndRetTrue, whileBody);
  ...
  case ValueType::Tuple: {
    ...
    for (const auto &child : schema->Children()) {
      ++childCount;
      childValidators.emplace_back(
          CreateTypeValidator(child->Schema, context, builder, module,
                              functions, suffix + std::to_string(childCount)));
    }
    auto func = llvm::Function::Create(
        funcType, llvm::Function::ExternalLinkage, funcName, module);
    ...
    for (int i = 0; i < childValidators.size(); ++i) {
      ...
      nextField = llvm::BasicBlock::Create(
          *context, "field" + std::to_string(i + 1), func);
      auto childCall =
          builder->CreateCall(childValidators[i], {func->arg_begin()});
      builder->CreateCondBr(childCall, nextField, retFalse);
    }
    builder->SetInsertPoint(nextField);
    auto callNextAndRetTrue =
...
orc::ThreadSafeModule FinalizeModule(std::unique_ptr<Module> module,
...
std::unique_ptr<orc::LLJIT> PrepareJit(PrepareJitFor prepareFor,
  ...
  case PrepareJitFor::Json: {
    symbolMap.insert({Mangle("FillWithEvents"),
                      JITEvaluatedSymbol(pointerToJITTargetAddress(
                                             &JsonValidators::FillWithEvents),
