...
public:
  ...
  bool Validate(jsoncons::json_cursor *cursor) override {
    ...
    if (ValueType_ == ValueType::Int) {
      return tp == jsoncons::staj_event_type::int64_value ||
             tp == jsoncons::staj_event_type::uint64_value;
...
public:
  explicit OptionalJsonconsValidator(
      std::unique_ptr<JsonconsCursorValidator> child)
...
public:
  explicit ListJsonconsValidator(std::unique_ptr<JsonconsCursorValidator> child)
      : ChildValidator_(std::move(child)) {}

  bool Validate(jsoncons::json_cursor *cursor) override {
    if (cursor->current().event_type() !=
...
public:
  explicit TupleJsonconsValidator(
...
public:
  explicit RootJsonconsValidator(std::unique_ptr<JsonconsCursorValidator> child)
      : ChildValidator_(std::move(child)) {}
...
CreateJsonconsCursorValidator(const TypeBasePtr &schema, int depth) {
  if (depth == 0) {
    return std::make_unique<RootJsonconsValidator>(
        CreateJsonconsCursorValidator(schema, depth + 1));
  }
  ...
  case ValueType::Optional: {
    return std::make_unique<OptionalJsonconsValidator>(
        CreateJsonconsCursorValidator(schema->Child(), depth + 1));
  }
  case ValueType::List: {
    return std::make_unique<ListJsonconsValidator>(
        CreateJsonconsCursorValidator(schema->Child(), depth + 1));
  }
  case ValueType::Tuple: {
    ...
    for (const auto &child : schema->Children()) {
      children.emplace_back(
          CreateJsonconsCursorValidator(child->Schema, depth + 1));
    }
